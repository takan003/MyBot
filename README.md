...
2. **設定4個連結**：打開主程式所在的試算表（我的聊天機器人_內含主程式_自由取名），裏頭有4個共用連結要填寫，分別為機器人圖像連結（圖片檔案）、成員名冊連結（試算表）、基本數據庫連結（試算表）、問題收集試算表連結（試算表），就是剛剛建立副本的那些檔案，其中圖片檔案的共用權限請設定為「知道連結的任何人：檢視者」。當然，可以順便修改其他的設定項目。
3. **部署程式**：打開主程式所在的試算表（我的聊天機器人_內含主程式_自由取名），在上方功能選單中找到擴充功能中的「Apps Script」，利用右上角部署按鈕，將程式部署為「網頁應用程式」，執行身分為「我」，誰可以存取「所有人」，部署好後取得網址就能執行了。
